...
from ttf.normalization_bias_diagnostic import normalization_bias_phase_redraw_scores
...
def run(rule: dict, *, cell_id: str, start: int, count: int) -> dict:
    ...
    if rule["terminal_rule"].get("candidate_selected") is not None:
        raise ValueError("normalization-bias diagnostic must remain non-candidate")
    if rule["terminal_rule"].get("formal_pass_fail") is not None:
        raise ValueError("normalization-bias diagnostic must remain non-qualifying")
    ...
    fixed = rule["estimators"]["normalized_q51"]
    ...
    for replicate in range(start, start + count):
        ...
        result = normalization_bias_phase_redraw_scores(
            samples,
            train_species=train,
        ...
        normalized = np.asarray(result.normalized_statistics, dtype=float)
        unscaled = np.asarray(result.unscaled_statistics, dtype=float)
        if normalized.shape != (n_phase,) or unscaled.shape != (n_phase,):
            raise RuntimeError("normalization-bias score width drift")
        if not np.isfinite(normalized).all() or not np.isfinite(unscaled).all():
            raise RuntimeError("normalization-bias scores must be finite")
        rows.append(
            {
                "replicate": int(replicate),
                "geometry_seed": int(geometry_seed),
                "phase_seed": int(phase_seed),
                "conditional_mean_normalized_C": float(np.mean(normalized)),
                "conditional_mean_unscaled_C": float(np.mean(unscaled)),
                "conditional_mean_normalized_minus_unscaled_C": float(np.mean(normalized - unscaled)),
                "within_geometry_phase_sd_normalized_C": float(np.std(normalized, ddof=1)),
                "within_geometry_phase_sd_unscaled_C": float(np.std(unscaled, ddof=1)),
                "phase_fraction_positive_normalized_C": float(np.mean(normalized > 0.0)),
                "phase_fraction_positive_unscaled_C": float(np.mean(unscaled > 0.0)),
