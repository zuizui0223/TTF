...
def main()->int:
    ...
    if any(bool(v) for v in rule["response_firewall"].values()):
        raise RuntimeError("Study-B empirical opening-rule firewall is open")
    ...
    if summary.get("status")!="PASS_TO_CONFIRMATORY_SURVIVOR_SYNTHETIC_REQUALIFICATION":
        raise RuntimeError("Study-B survivor summary is not eligible for requalification")
    if mask.get("status")!="PASS_TO_CONFIRMATORY_SURVIVOR_REQUALIFICATION":
        raise RuntimeError("Study-B mask result did not pass to survivor requalification")
    if qual.get("schema")!="ttf_relational_environment_qualification_result_v0.2":
        raise RuntimeError("unexpected Study-B survivor qualification schema")
    if qual.get("qualification_stage")!="confirmatory_survivor":
        raise RuntimeError("Study-B qualification is not survivor-stage")
    ...
    if qual.get("gates",{}).get("overall_pass") is not True:
        raise RuntimeError("Study-B survivor requalification overall gate is not PASS")
    ...
    if summary.get("design_npz_sha256")!=design_sha:
        raise RuntimeError("Study-B survivor design hash drift")
    if qual.get("opportunity_design_sha256")!=design_sha:
        raise RuntimeError("Study-B survivor qualification design hash drift")
    ...
    code_paths=[
        Path("scripts/run_relational_environment_empirical.py"),
    ...
    if missing:
        raise FileNotFoundError(f"missing frozen empirical code: {missing}")

    payload={
        "schema":"ttf_relational_environment_identity_opening_authorization_v0.1",
        "status":"AUTHORIZE_ONE_SHOT_STUDY_B_NUCLEOTIDE_IDENTITY_OPENING",
        "source":{
