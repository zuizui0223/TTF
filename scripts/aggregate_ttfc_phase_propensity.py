...
WORLD_KEYS = (
    "conditional_mean_baseline_C",
    "conditional_mean_propensity_centered_C",
    "conditional_mean_baseline_minus_centered_C",
    "within_geometry_phase_sd_baseline_C",
    "within_geometry_phase_sd_propensity_centered_C",
    "phase_fraction_positive_baseline_C",
    "phase_fraction_positive_propensity_centered_C",
    "mean_training_expected_residual_rms",
)
...
def _collect(rule: dict, paths: list[Path]) -> dict[str, dict[str, np.ndarray]]:
    ...
    for path in paths:
        ...
        if payload.get("schema") != "ttfc_phase_propensity_diagnostic_batch_v0.1":
        ...
        if payload.get("candidate_selected") is not None or payload.get("formal_pass_fail") is not None:
            raise ValueError("phase-propensity batch attempted selection or qualification")
...
def _mechanism_for_cell(summary: dict) -> dict:
    baseline = summary["endpoints"]["conditional_mean_baseline_C"]
    centered = summary["endpoints"]["conditional_mean_propensity_centered_C"]
    difference = summary["endpoints"]["conditional_mean_baseline_minus_centered_C"]
    baseline_bias = bool(baseline["low95"] > 0.0)
    contribution = bool(difference["low95"] > 0.0)
    recentered = bool(centered["low95"] <= 0.0 <= centered["high95"])
    return {
        "baseline_bias_replication": {
            "supported": baseline_bias,
            "conditional_mean_baseline_C": baseline,
        },
        "phase_propensity_contribution": {
            "supported": contribution,
            "baseline_minus_centered_C": difference,
        },
        "propensity_centered_recentered": {
            "supported": recentered,
            "conditional_mean_propensity_centered_C": centered,
        },
        "joint_supported": bool(baseline_bias and contribution and recentered),
    }
...
def aggregate(rule: dict, batch_paths: list[Path]) -> dict:
    ...
    for i, cell in enumerate(cfg["cells"]):
        ...
        summaries[cid] = {
            ...
            "role": cell["role"],
            "n_geometry_worlds": int(len(cells[cid]["conditional_mean_baseline_C"])),
            "endpoints": {
                key: _bootstrap_mean(
                    cells[cid][key],
    ...
    matched = _mechanism_for_cell(summaries["DPP_MATCHED"])
    shifted = _mechanism_for_cell(summaries["DPP_SHIFTED"])

    return {
        "schema": "ttfc_phase_propensity_diagnostic_result_v0.1",
        "status": "diagnostic_complete_nonqualifying",
        "candidate_selected": None,
        "formal_pass_fail": None,
        "cell_summaries": summaries,
        "mechanism_tests": {
            "matched_baseline_bias_replication": matched["baseline_bias_replication"],
            "matched_phase_propensity_contribution": matched["phase_propensity_contribution"],
            "matched_propensity_centered_recentered": matched["propensity_centered_recentered"],
            ...
            "shifted_baseline_bias_replication": shifted["baseline_bias_replication"],
            "shifted_phase_propensity_contribution": shifted["phase_propensity_contribution"],
            "shifted_propensity_centered_recentered": shifted["propensity_centered_recentered"],
            ...
            "homogeneous_negative_control": {
                "baseline": summaries["DPP_HOMOGENEOUS"]["endpoints"]["conditional_mean_baseline_C"],
                "propensity_centered": summaries["DPP_HOMOGENEOUS"]["endpoints"]["conditional_mean_propensity_centered_C"],
                "baseline_minus_centered": summaries["DPP_HOMOGENEOUS"]["endpoints"]["conditional_mean_baseline_minus_centered_C"],
                "expected_residual_rms": summaries["DPP_HOMOGENEOUS"]["endpoints"]["mean_training_expected_residual_rms"],
            },
        },
        "global_gate_predeclared": False,
        "interpretation": "This diagnostic tests whether the exact geometry-only phase-expected unscaled training residual is necessary for the supported heterogeneous TTF-C conditional-null bias. It does not select the propensity-centered ablation or any correction.",
...
def main() -> None:
    ...
    if not paths:
        raise ValueError("no phase-propensity batches found")
    ...
    print(
        json.dumps(
            {
                "primary_phase_propensity_mechanism": result["mechanism_tests"]["primary_phase_propensity_mechanism"]["supported"],
                "shifted_joint_phase_propensity_mechanism": result["mechanism_tests"]["shifted_joint_phase_propensity_mechanism"]["supported"],
                "candidate_selected": None,
                "formal_pass_fail": None,
            },
            sort_keys=True,
