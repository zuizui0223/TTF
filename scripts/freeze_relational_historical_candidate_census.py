...
Response-blind only: verified prior design membership, taxonomy, marker labels,
FASTA headers, occurrence coordinates, and graph geometry. Nucleotide identity
and empirical transfer responses remain closed.
"""
...
def main() -> int:
    ...
    ap.add_argument("--s1-exclusion-manifest", type=Path, required=True)
    ap.add_argument("--s2-exclusion-manifest", type=Path, required=True)
