...
def main()->int:
    ...
    if rule.get("schema")!="ttf_relational_historical_climate_character_mask_rule_v0.1":
        raise RuntimeError("unexpected Study-C mask rule")
    if any(bool(v) for v in rule["response_firewall"].values()):
        raise RuntimeError("Study-C mask-rule firewall is open")

    opportunity=json.loads(args.opportunity_summary.read_text())
    if opportunity.get("status")!="PASS_TO_HISTORICAL_DEVELOPMENT_SYNTHETIC_QUALIFICATION":
        raise RuntimeError("Study-C opportunity design did not pass")
    ...
    if qualification.get("schema")!="ttf_relational_historical_climate_qualification_result_v0.1":
        raise RuntimeError("unexpected Study-C development qualification result")
    if qualification.get("status")!="PASS_TO_HISTORICAL_CONFIRMATORY_CHARACTER_MASK_PREPARATION":
        raise RuntimeError("Study-C development qualification did not authorize mask opening")
    if qualification.get("gates",{}).get("overall_pass") is not True:
        raise RuntimeError("Study-C development qualification overall gate is not PASS")
    if any(bool(v) for v in qualification["response_firewall"].values()):
        raise RuntimeError("Study-C development qualification firewall is open")
    if qualification.get("opportunity_design_sha256")!=sha256_path(args.opportunity_design):
        raise RuntimeError("Study-C opportunity-design hash drift at mask opening")
    ...
    arrays={
        "R_hist":np.asarray(data["confirmatory_R_hist"],float),
        "R_current":np.asarray(data["confirmatory_R_current"],float),
    ...
    n=len(source)
    if len(target)!=n or any(len(v)!=n for v in arrays.values()):
        raise RuntimeError("Study-C confirmatory opportunity-array width drift")
    ...
    if not set(empirical_union).issubset(metadata):
        raise RuntimeError("Study-C confirmatory species absent from frozen candidate metadata")

    fraction=float(rule["inherited_parser_contract"]["minimum_comparable_fraction"])
    if fraction!=0.50:
        raise RuntimeError("Study-C comparable-site fraction drift")
    ...
    for i,name in enumerate(empirical_union,1):
        ...
        if not fasta.is_file() or not occurrence.is_file():
            raise RuntimeError(f"missing frozen Study-C source file for {name}")
        try:
    ...
    minimum_sources=int(rule["post_mask_dyad_support"]["minimum_surviving_sources_per_target"])
    ...
    np.savez_compressed(
        ...
        survivor_R_hist=survivor_arrays["R_hist"],
        survivor_R_current=survivor_arrays["R_current"],
    ...
    mask_payload={
        "schema":"ttf_relational_historical_climate_character_mask_result_v0.1",
        "status":"PASS_TO_HISTORICAL_CONFIRMATORY_SURVIVOR_REQUALIFICATION",
    ...
    summary={
        "schema":"ttf_relational_historical_climate_survivor_opportunity_design_v0.1",
        "status":"PASS_TO_HISTORICAL_CONFIRMATORY_SURVIVOR_SYNTHETIC_REQUALIFICATION",
        "opportunity_design_sha256":sha256_path(args.opportunity_design),
