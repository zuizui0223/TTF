...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate joint training-strength and raw-transfer statistics for v0.7."
    )
    ...
    parser.add_argument("--strength-neighbours", type=int, default=4)
    ...
    if args.mode == "reference":
        if not np.isclose(args.shared_fraction, 0.0):
            raise RuntimeError("v0.7 private reference must have shared_fraction=0")
    ...
    for batch_start in range(0, args.replicates, args.world_batch_size):
        ...
        for replicate in range(batch_start, batch_stop):
            ...
            for name in train:
                train_columns[name].append(train_response[name])
    ...
    if len(t) != args.replicates or u.shape != t.shape:
        raise RuntimeError("v0.7 joint statistic generation was incomplete")
    if not np.isfinite(t).all() or not np.isfinite(u).all():
        raise RuntimeError("v0.7 joint statistics contain non-finite values")
    ...
    if args.mode == "observed":
        ...
        baseline_reproduced = bool(
            np.isclose(t.mean(), float(baseline["mean_statistic"]), atol=1e-12, rtol=0.0)
        )
        if not baseline_reproduced:
            raise RuntimeError(
                f"raw statistic drift: {t.mean()} != {baseline['mean_statistic']}"
            )

    payload = {
        "schema": "ttf_v07_profiled_statistics_v0.1",
        ...
        "config": {
            ...
            "strength": "equal_species_mean_training_edge_midpoint_neighbor_coherence",
        },
        ...
        "summary": {
            ...
            "strength_q05": float(np.quantile(u, 0.05)),
            "strength_q95": float(np.quantile(u, 0.95)),
            "strength_statistic_correlation": float(np.corrcoef(u, t)[0, 1]),
        },
        "baseline_v05_reproduced": baseline_reproduced,
        "heldout_trait_used_for_strength": False,
