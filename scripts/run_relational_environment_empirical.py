...
def main()->int:
    ...
    if auth.get("schema")!="ttf_relational_environment_identity_opening_authorization_v0.1":
        raise RuntimeError("unexpected Study-B identity-opening authorization")
    if auth.get("status")!="AUTHORIZE_ONE_SHOT_STUDY_B_NUCLEOTIDE_IDENTITY_OPENING":
        raise RuntimeError("Study-B identity opening is not authorized")
    ...
    if actual_inputs!=auth["inputs_sha256"]:
        raise RuntimeError("Study-B authorized input hash drift")
    if args.source_archive_sha256!=auth["source"]["archive_sha256"]:
        raise RuntimeError("Study-B source archive SHA drift")

    root=args.root.resolve()
    if sha256_path(root/"genes.txt")!=auth["source"]["genes_txt_sha256"]:
        raise RuntimeError("genes.txt drift since Study-B authorization")
    if sha256_path(root/"cite.txt")!=auth["source"]["cite_txt_sha256"]:
        raise RuntimeError("cite.txt drift since Study-B authorization")
    for path,expected in auth["code_sha256"].items():
        p=Path(path)
        if not p.is_file() or sha256_path(p)!=expected:
            raise RuntimeError(f"Study-B authorized code drift: {path}")

    rule=json.loads(args.opening_rule.read_text())
    if rule.get("schema")!="ttf_relational_environment_empirical_opening_rule_v0.1":
        raise RuntimeError("unexpected Study-B empirical opening rule")
    ...
    for payload in (mask,summary,qual):
        if any(bool(v) for v in payload["response_firewall"].values()):
            raise RuntimeError("Study-B pre-identity response firewall is open")
    if qual.get("status")!="PASS_TO_EMPIRICAL_IDENTITY_OPENING_PREPARATION":
        raise RuntimeError("Study-B survivor requalification did not pass")
    ...
    r_env=np.asarray(data["survivor_R_env"],float)
    ...
    n=len(s)
    if any(len(x)!=n for x in (t,r_env,coverage,same_class,same_order,same_family,locality_ratio)):
        raise RuntimeError("Study-B survivor dyad-array drift")
    ...
    if checks!=frozen:
        raise RuntimeError("Study-B authorized empirical design drift")
    if set(source_names)&set(target_names):
        raise RuntimeError("Study-B source/target role overlap")
    ...
    if not set(empirical_species).issubset(metadata) or not set(empirical_species).issubset(mask_by_species):
        raise RuntimeError("Study-B empirical species missing frozen metadata/mask")
    ...
    # First authorized nucleotide-identity opening for Study B.
    response_map={}
    pair_count_summary={}
    for i,name in enumerate(empirical_species,1):
        ...
        if mask_row.get("status")!="PASS_MASK":
            raise RuntimeError(f"Study-B empirical species lacks PASS mask: {name}")
        ...
        if canonical_mask_sha256_from_identity(alignment)!=str(mask_row["mask_sha256"]):
            raise RuntimeError(f"Study-B canonical mask digest drift: {name}")
        grouped=sequences_by_frozen_locality(
            alignment,
            read_occurrence_rows(occurrence),
            canonical_latlon[name],
        )
    ...
    if not np.isclose(prepared.condition_number,expected_condition,rtol=0.0,atol=1e-9):
        raise RuntimeError("Study-B authorized predictor condition-number drift")
    ...
    decision=(
        "STUDY_B_ENVIRONMENT_RELATIONAL_POSITIVE"
        if positive else
    ...
    dyads=[
        ...
        for a,b,score,rr,gg,sc,so,sf,lr in zip(
            s,t,t_st,r_env,coverage,same_class,same_order,same_family,locality_ratio
        )
    ]
    payload={
        "schema":"ttf_relational_environment_empirical_result_v0.1",
        "status":"EMPIRICAL_STUDY_B_RESULT_OPENED_UNDER_FROZEN_AUTHORIZATION",
        "authorization_sha256":sha256_path(args.authorization),
        "input_sha256":actual_inputs,
        "empirical_design":checks,
        "primary":{
            "estimand":"beta_R for z_R_env",
            "coefficient":beta,
        ...
        "dyads":dyads,
        "outcome_state":{
            "Study_B_sequence_identity_opened":True,
            "Study_B_pairwise_genetic_distances_computed_in_memory":True,
            "serialized_sequence_identity":False,
            "serialized_edge_genetic_distance_vectors":False,
            "Study_B_T_st_computed":True,
            "Study_B_beta_R_computed":True,
        },
        "one_shot":{
    ...
    print(json.dumps({
        "decision":decision,"beta_R":beta,"se":se,"z":z,"p":p,"dyads":len(t_st)
    },sort_keys=True))
    return 0
