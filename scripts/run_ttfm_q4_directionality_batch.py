...
def run(rule: dict, *, cell_id: str, start: int, count: int) -> dict:
    ...
    if len(matches) != 1:
        raise ValueError(f"unknown or duplicate Q4 cell: {cell_id}")
    ...
    for replicate in range(start, start + count):
        ...
        if len(labels) != n_train + n_eval:
            raise RuntimeError("Q4 split size does not match frozen system count")
    ...
    return {
        "schema": "ttfm_q4_directionality_batch_v0.1",
