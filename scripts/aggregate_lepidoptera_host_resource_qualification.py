...
from ttf.lepidoptera_host_resource_qualification import nuisance_envelope_pvalues
...
def collect(directory:Path,namespace:str,cell:str,expected:int)->np.ndarray:
    ...
    for path in sorted(directory.glob("*.json")):
        ...
        if p.get("schema")!="ttf_lepidoptera_host_resource_qualification_shard_v0.1":
            continue
...
def main()->int:
    ...
    ref_private=collect(args.input_dir,"reference","private",499)
    ref_geometry=collect(args.input_dir,"reference","geometry_confounded_trap",499)
    ref_breadth=collect(args.input_dir,"reference","host_breadth_confounded_trap",499)
    ...
    for cell in CELLS:
        ...
        p=nuisance_envelope_pvalues(
            values,
            private_reference=ref_private,
            geometry_reference=ref_geometry,
            breadth_reference=ref_breadth,
        )
        rejected=p<=.05
        k=int(np.count_nonzero(rejected))
        lo,hi=wilson_interval(k,len(values))
        evaluation[cell]={
            "worlds":int(len(values)),
            "rejections":k,
    ...
    out={
        "schema":"ttf_lepidoptera_host_resource_survivor_qualification_result_v0.1",
        ...
        "reference":{
            ...
            "host_breadth_mean_statistic":float(np.mean(ref_breadth)),
        },
