...
def main() -> int:
    ...
    for path in args.shard:
        ...
        if shard.get("status") != "SYNTHETIC_QUALIFICATION_SHARD_COMPLETE":
    ...
    payload = {
        "schema": "ttf_genetic_conditional_order_qualification_v0.1",
        "status": "PASS" if passed else "NOT_EVALUABLE_CONDITIONAL_ORDER",
        "passed": passed,
        ...
        "next_step": (
            "qualification method PASS only; confirmatory character-mask admissibility and exact survivor-geometry requalification are still required before any nucleotide identity opening"
            if passed
