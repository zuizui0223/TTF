...
def main() -> int:
    ...
    parser.add_argument(
        "--rule",
        type=Path,
        default=Path("docs/supporting/genetic_conditional_order_contrast_qualification_rule_v0.2.json"),
    )
    ...
    if rule.get("status") != "FROZEN_BEFORE_V02_FORMAL_QUALIFICATION_AND_ANY_CONDITIONAL_EMPIRICAL_IDENTITY":
        raise RuntimeError("v0.2 qualification rule is not frozen pre-outcome")
    ...
    expected_supported = int(rule["development_panel"]["jointly_supported_eval_species"])
    if len(design.eligible_eval_species) != expected_supported:
        raise RuntimeError(
            f"response-blind jointly supported target count drift: "
            f"{len(design.eligible_eval_species)} != {expected_supported}"
        )
