...
def main() -> int:
    ...
    if authorization.get("schema") != "ttf_relational_host_resource_identity_opening_authorization_v0.1":
        raise RuntimeError("unexpected relational identity-opening authorization")
    if authorization.get("status") != "AUTHORIZE_RELATIONAL_HOST_RESOURCE_NUCLEOTIDE_IDENTITY_OPENING":
        raise RuntimeError("relational empirical opening is not authorized")
    ...
    survivors = read_survivors(args.survivors)
    if species_digest(list(survivors)) != authorization["species"]["survivor_digest_sha256"]:
        raise RuntimeError("survivor species digest drift")
    ...
    # This is the first authorized identity-opening boundary.
    response_map = {}
    pair_count_summary = {}
    for name in empirical_species:
        panel = panel_map[name]
    ...
    payload = {
        "schema": "ttf_relational_host_resource_empirical_result_v0.1",
        "status": "EMPIRICAL_RELATIONAL_RESULT_OPENED_UNDER_FROZEN_AUTHORIZATION",
