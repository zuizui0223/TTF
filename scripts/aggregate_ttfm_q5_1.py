...
def aggregate(rule: dict, batch_paths: list[Path]) -> dict:
    if rule["status"] != "frozen_before_q5_1_outcomes":
        raise ValueError("Q5.1 rule is not frozen")
    ...
    for path in batch_paths:
        ...
        if payload.get("schema") != "ttfm_q5_1_batch_v0.1":
    ...
    for cell_id, cell in cells.items():
        ...
        raw_c_reject = sum(float(row["raw_ttf_c_p_value"]) <= alpha for row in rows)
        m_low, m_high = wilson_interval(m_reject, expected_n)
        c_low, c_high = wilson_interval(c_reject, expected_n)
        raw_c_low, raw_c_high = wilson_interval(raw_c_reject, expected_n)
        cell_results.append(
            {
                "id": cell_id,
                ...
                "TTF_C_RAW_AUDIT": {
                    "rejections": int(raw_c_reject),
                    "rejection_rate": float(raw_c_reject / expected_n),
                    "wilson95_low": float(raw_c_low),
                    "wilson95_high": float(raw_c_high),
                    "mean_statistic": float(sum(float(r["raw_ttf_c_statistic"]) for r in rows) / expected_n),
                    "decision_role": "audit_only",
                },
                "geometry_summary": {
    ...
    def estimand_pass(result_key: str, role_prefix: str) -> tuple[bool, list[dict]]:
        checks = []
        type_role = f"{role_prefix}_type1"
        power_role = f"{role_prefix}_power"
        for result in cell_results:
            roles = set(result["roles"])
            if type_role in roles:
                observed = float(result[result_key]["wilson95_high"])
                checks.append({
                    "cell_id": result["id"],
                    "role": "type1",
            ...
            if power_role in roles:
                observed = float(result[result_key]["wilson95_low"])
                checks.append({
                    "cell_id": result["id"],
                    "role": "power",
                    "observed_bound": observed,
    ...
    m_pass, m_checks = estimand_pass("TTF_M", "ttf_m")
    c_pass, c_checks = estimand_pass("TTF_C_Q51", "ttf_c")
    return {
        "schema": "ttfm_q5_1_coupling_geometry_result_v0.1",
        "status": "formal_q5_1_complete",
        "rule_schema": rule["schema"],
        "TTF_M_Q5_1_pass": bool(m_pass),
        "TTF_C_Q5_1_pass": bool(c_pass),
        "joint_Q5_1_pass": bool(m_pass and c_pass),
        "TTF_M_checks": m_checks,
        "TTF_C_Q5_1_checks": c_checks,
        "cells": cell_results,
        "claim_firewall": rule["claim_firewall"],
        "q5_result_remains_immutable_fail": True,
        "next_if_pass": "independent_observation_support_contract_before_any_empirical_application",
        "next_if_fail": "freeze_q5_1_fail_and_require_new_named_successor_with_fresh_worlds",
    }
...
def main() -> None:
    ...
    if not batch_paths:
        raise ValueError("no Q5.1 batch files found")
    result = aggregate(rule, batch_paths)
    out = Path(args.output)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({
        "TTF_M_Q5_1_pass": result["TTF_M_Q5_1_pass"],
        "TTF_C_Q5_1_pass": result["TTF_C_Q5_1_pass"],
        "joint_Q5_1_pass": result["joint_Q5_1_pass"],
    }, sort_keys=True))
