...
def cell_key(shared: float, amplitude: float) -> str:
    return f"shared_{shared:g}__amplitude_{amplitude:g}"
...
def main() -> int:
    ...
    expected_n = int(pilot['qualification']['observed_worlds_per_cell'])
    ...
    if not files:
        raise RuntimeError('no observed shard files found')
    for path in files:
        shard = json.loads(path.read_text())
        if shard.get('schema') != 'ttf_genetic_gate_d_observed_shard_v0.1':
            continue
        ...
        if key not in grouped:
            raise RuntimeError(f'unexpected observed cell {key}')
        for row in shard['rows']:
            replicate = int(row['replicate'])
            if replicate in grouped[key]:
                raise RuntimeError(f'duplicate observed replicate {key}:{replicate}')
    ...
    for shared, amplitude in expected_cells:
        ...
        if sorted(rows) != list(range(expected_n)):
            ...
            raise RuntimeError(f'incomplete observed cell {(shared,amplitude)}; first missing={missing}')
    ...
    type1_ceiling = float(pilot['qualification']['type1_wilson95_upper_ceiling'])
    power_floor = float(pilot['qualification']['shared_A2_wilson95_lower_floor'])
    ...
    out = {
        'schema': 'ttf_genetic_gate_d_qualification_v0.1',
        ...
        'passed': passed,
        'failure_interpretation': pilot['qualification']['failure_interpretation'],
        'empirical_pilot_opening_eligible': passed,
        'empirical_genetic_outcomes_opened': False,
        'claim_boundary': (
            'PASS authorizes only a separate, explicit Decker empirical pilot opening step. '
            'It is not a fresh confirmatory biological result. NOT_EVALUABLE is not evidence '
            'against shared phylogeography.'
        ),
    }
