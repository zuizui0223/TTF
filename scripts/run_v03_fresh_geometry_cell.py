...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run one locked v0.3 fresh external geometry qualification cell."
    )
    ...
    if source.get("validation_role") != "prospectively_frozen_fresh_external_geometry_for_v03_only":
        raise RuntimeError("source ledger is not the prospective v0.3 fresh holdout")
    if source.get("confirmatory_holdout") is not True:
        raise RuntimeError("fresh geometry is not marked confirmatory holdout")
    if source.get("candidate_performance_evaluated_at_freeze") is not False:
        raise RuntimeError("fresh geometry outcome firewall was not preserved")
    if int(source.get("synthetic_worlds_run_at_freeze", -1)) != 0:
        raise RuntimeError("fresh geometry had synthetic outcomes before candidate lock")
    if source.get("failed_i_a_taxa_overlap") != []:
        raise RuntimeError("fresh geometry overlaps failed I-A taxa")
    prospective = source.get("prospective_execution", {})
    if int(prospective.get("k")) != int(args.k):
        raise RuntimeError("fresh k differs from pre-outcome geometry ledger")
    if abs(float(prospective.get("bandwidth_km")) - float(args.bandwidth)) > 1e-12:
        raise RuntimeError("fresh bandwidth differs from pre-outcome geometry ledger")
    if abs(float(prospective.get("eval_fraction")) - float(args.eval_fraction)) > 1e-12:
        raise RuntimeError("fresh eval fraction differs from pre-outcome geometry ledger")
    if int(prospective.get("seed")) != int(args.seed):
        raise RuntimeError("fresh seed differs from pre-outcome geometry ledger")
    ...
    if development.get("selected_candidate") != V03_LOCKED_CANDIDATE:
        raise RuntimeError("code candidate differs from frozen development selection")
    if development.get("fresh_external_qualification_required") is not True:
        raise RuntimeError("development result did not require fresh validation")
    if development.get("failed_panel_cannot_qualify_selected_candidate") is not True:
        raise RuntimeError("failed-panel claim firewall is missing")
    ...
    if excluded:
        raise RuntimeError(f"fresh qualification unexpectedly excluded taxa: {excluded}")
    if len(geometries) != 40:
        raise RuntimeError(f"fresh qualification requires exactly 40 taxa, found {len(geometries)}")
    ...
    if len(train) != 20 or len(evaluation) != 20:
        raise RuntimeError("fresh v0.3 qualification requires fixed 20/20 species split")
    ...
    if len(cells) != 1:
        raise RuntimeError("fresh v0.3 cell runner produced unexpected grid")

    record_counts = {item.species: len(item.coordinates) for item in geometries}
    payload = {
        "schema": "ttf_v03_fresh_geometry_cell_v0.1",
        "candidate": V03_LOCKED_CANDIDATE,
        "qualification_role": "fresh_external_confirmatory_geometry",
        "geometry": {
